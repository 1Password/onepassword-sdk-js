...
export interface ReportsApi {
  ...
  getDevicesOverview(): Promise<types.DevicesOverview>;
}
...
export class ReportsSource implements ReportsApi {
  ...
  public async getDevicesOverview(): Promise<types.DevicesOverview> {
    const invocationConfig: InvokeConfig = {
      clientId: this.#inner.id,
      invocation: {
        name: "GetDevicesOverview",
        parameters: {},
      },
    };
    return JSON.parse(
      await this.#inner.core.invoke(invocationConfig),
    ) as Promise<types.DevicesOverview>;
  }
}
