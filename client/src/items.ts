...
import { OpList } from "./iterator.js";
...
export interface ItemsApi {
   ...
   *  List all items
   */
  list(vaultId: string): Promise<OpList<types.ItemOverview>>;
}
...
export class ItemsSource implements ItemsApi {
   ...
   *  List all items
   */
  public async list(vaultId: string): Promise<OpList<types.ItemOverview>> {
    const invocationConfig: InvokeConfig = {
      clientId: this.#inner.id,
      invocation: {
        name: "ItemsList",
    ...
    return new OpList<types.ItemOverview>(
      JSON.parse(await this.#inner.core.invoke(invocationConfig)),
    );
  }
}
