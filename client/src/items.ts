...
export interface ItemsApi {
   ...
   *  List all items
   */
  list(vaultId: string): Promise<SdkList<types.ItemOverview>>;
}
...
export class ItemsSource implements ItemsApi {
   ...
   *  List all items
   */
  public async list(vaultId: string): Promise<SdkList<types.ItemOverview>> {
    const invocationConfig: InvokeConfig = {
      clientId: this.#inner.id,
      invocation: {
        name: "ItemsList",
    ...
    return new SdkList<types.ItemOverview>(
