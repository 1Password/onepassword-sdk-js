...
build=$(awk -F "['\"]" '/SDK_BUILD_NUMBER =/{print $2}' "client/version.ts" | tr -d '\n')
version_sdk=$(awk -F "['\"]" '/SDK_VERSION =/{print $2}' "client/version.ts"| tr -d '\n')
version_sdk_core=$(awk -F "['\"]" '/SDK_CORE_VERSION =/{print $2}' "client/version.ts" | tr -d '\n')
